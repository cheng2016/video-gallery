...
- **一键转码**（动态站，需 ffmpeg）：m3u8 / TS 可转同目录 MP4；也可选输出格式、降到 1080/720、以及 2K/4K 高帧率片降到 60/30 等  
  - RMVB 需 ffmpeg 带 RealMedia；没有时会提示，不会假排队  
...
- **疑似重复提示**：同名或同体积的片在封面角标「重复」；后台轻量发布列表后会立刻补回角标，不用等整库重算  
...
- **时长进列表**：自动补时长，可按「最长 / 最短」排序；封面角标优先显示时长，静默刷新也会把已加载卡片的时长/重复/损坏角标对齐
- **扫描不打断刷片**：扫盘时封面墙静默增量刷新；目录变大时按你选的排序补齐，已滚下去的列表不会被缩回首页  
...
| 操作 | 行为 |
|------|------|
| 重启 / 扫描所选项 | 读 `catalog.sqlite` 缓存索引，不重复截图 |
| 重新扫描 | 全盘重扫；已有预览图仍复用 |
| 导出静态站 | 已有缩略图直接复用 |
| 删了的视频 | 加载时自动从列表去掉 |
